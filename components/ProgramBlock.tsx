...
import { ProgramListing } from "@/types/api";
    ...
    <div className="flex flex-col lg:flex-row gap-10 py-5 lg:py-10" key={index}>
      ...
      <div className={`w-full lg:w-[60%] flex flex-col gap-5 ${(index % 2) ? 'md:order-1' : 'order-2'}`}>
        ...
        }
        {
          program.program_highlights && program.program_highlights.length > 0 && (
            <div className="flex flex-col gap-2">
              ...
              <ul className="flex flex-col gap-3 text-base text-[#4E4E4E] list-disc list-outside pl-5">
                {
        ...
        }
        {
          program.program_career_paths && program.program_career_paths.length > 0 && (
            <div className="flex flex-col gap-2">
              ...
              <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-3 xl:gap-5 text-base text-[#4E4E4E] list-disc list-outside h-50 overflow-hidden overflow-y-auto lg:h-auto pl-5">
                {
        ...
        <div className="flex gap-5 text-burgundy">
          <Link href={`${basePath}programs/${program.program_url_slug}`}>Learn More About This Course <MdArrowOutward size={15} className="inline" /></Link>
        </div>
      </div>
    </div>
  )
