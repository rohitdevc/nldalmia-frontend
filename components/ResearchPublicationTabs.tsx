...
export default function ResearchPublicationTabs({page_name}: ResearchPublicationTabProps) {
    ...
    return (
        <ul className="flex flex-wrap gap-5 lg:gap-10 justify-center items-center text-center px-5 sm:px-10 md:px-15 xl:px-20 2xl:px-30 py-10" id="research-publication">
            {
                tabs.map((tab, key) => (
                    <li key={key}>
                        <Link href={`${basePath}faculty/research-papers-published/${tab.tab_url_slug}#research-publication`} className="relative group">
