...
export default function IQACsComponent() {
  ...
  const iqac_pdfs = [
    {
      iqac_category_title: 'NAAC Certificate',
      iqac_pdf_title: 'NAAC Cycle 1',
      iqac_pdf_file: 'NAAC-Cycle-2-1-scaled.jpg'
    },
    {
      iqac_category_title: 'NAAC Certificate',
      iqac_pdf_title: 'NAAC Cycle 2',
      iqac_pdf_file: 'NAAC-Cycle-2-1-scaled.jpg'
    },
    {
      iqac_category_title: 'NAAC SSR',
      iqac_pdf_title: 'NAAC SSR Cycle 2',
      iqac_pdf_file: 'NAAC-SSR-Cycle-2.pdf'
    },
    {
      iqac_category_title: 'NAAC SSR',
      iqac_pdf_title: 'NAAC SSR Cycle 2',
  ...
  return (
    ...
    <main className="w-full" style={{backgroundImage: `url(${basePath}images/home/bg-pattern.png)`}}>
      ...
      <div className="w-full px-15 xl:px-30 py-10 relative">
        ...
        <Swiper modules={[Navigation]} slidesPerView={3} spaceBetween={0} navigation={{prevEl: '.iqac_categories_slider_prev', nextEl: '.iqac_categories_slider_next'}} breakpoints={{640: {slidesPerView: 4}, 768: {slidesPerView: 5}, 1024: {slidesPerView: 6}, 1280: {slidesPerView: 8}}}>
          {
          ...
          <SwiperSlide className="text-center" key={iqac_categories.length}>
        ...
        <div className="w-full flex flex-col gap-5 px-5 md:px-15 xl:px-30 pb-10">
          {
            iqac_pdfs.filter(iqac_pdf =>
              ...
              <Link className="w-full flex justify-between items-center py-5 border-b border-[#800000] last:border-b-0" href={`${basePath}pdfs/${iqac_pdf.iqac_pdf_file}`} target="_blank" key={key} title={iqac_pdf.iqac_pdf_title}>
                <span className="font-georgia text-xl">{iqac_pdf.iqac_pdf_title}</span>
          ...
          {
            activeIQACCategoryName === "POE" && [...Array(5)].map((_, i) => (
              <div className="w-full flex flex-col gap-5 py-5 border-b border-[#800000] last:border-b-0" key={i}>
                <h2 className="font-georgia text-xl">PEO {i}</h2>
                <p className="text-sm leading-loose">To impart knowledge, analytical ability and management skills to participating PGDM students</p>
              </div>
            ))
          }
        </div>
