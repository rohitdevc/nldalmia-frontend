                      ...
                      <div className="flex flex-col gap-5">
                        ...
                        }
                        {
                          international_partner.program_international_partner_description && (
                            <p className="h-40 overflow-y-auto scrollbar-thin scrollbar-thumb-black/60 hover:scrollbar-thumb-black/60">{parser(nl2br(international_partner.program_international_partner_description))}</p>
                          )
                        }
                      </div>
                    </div>
            ...
            <Swiper modules={[Navigation, Autoplay]} autoplay={{delay: 5000, disableOnInteraction: false, pauseOnMouseEnter: true}} className="w-full" slidesPerView={1} spaceBetween={20} autoHeight={false} loop={true} navigation={{prevEl: '.program_outcome_prev', nextEl: '.program_outcome_next'}} breakpoints={{640: {slidesPerView: 2}, 768: {slidesPerView: 2}, 1024: {slidesPerView: 3}}}>
              {
                program.program_outcomes.map((program_outcome, key) => (
                  <SwiperSlide className="!h-auto flex" title={program_outcome.program_outcome_caption} key={key}>
                    <div className="w-full h-full bg-[#FFCC33] py-5 px-4 flex-1 flex flex-col gap-10 text-center">
                      <h3 className="text-xl font-georgia">{program_outcome.program_outcome_caption}</h3>
                      {
                        program_outcome.program_outcome_description && (
                          <p className="text-[#4E4E4E]">{parser(nl2br(program_outcome.program_outcome_description))}</p>
                        )
                      }
                    </div>
                  </SwiperSlide>
